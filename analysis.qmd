...
# ============================================================
# GUIDANCE FOR WRITTEN COMMENTARY (Parts e & f)
# ============================================================
#
# ----- PART E: Comparing the four designs -----
#
# POINT ESTIMATES (proportions):
#   Proportions will shift between Design 1 (SRS) and Designs 2-4 because
#   anweight corrects for unequal selection probabilities and non-response.
#   If certain groups are under-represented in the raw sample, weighting
#   adjusts for this.
#   - env_steps is an attitudinal variable: weighting may shift the Yes%
#     if environmentally engaged groups are over- or under-sampled.
#   - citizen is a demographic variable closely tied to the sampling
#     frame: expect a larger shift from weighting here.
#
# STANDARD ERRORS:
#   Design 1 (SRS):              Smallest SEs — falsely precise, ignores design.
#   Design 2 (weight only):      SEs increase because unequal weights add variance.
#   Design 3 (weight + strata):  SEs may fall relative to Design 2 — stratification
#                                 removes between-stratum variance.
#   Design 4 (full design):      Typically the largest SEs — adding PSU clustering
#                                 means observations within a PSU are correlated,
#                                 so we have fewer effectively independent units.
#
# ----- PART F: DEFF, DEFT, and Neff -----
#
# DEFF > 1: complex design is LESS efficient than SRS (common with clustering).
# DEFF < 1: complex design is MORE efficient than SRS (can occur with strong
#           stratification alone, as in the seminar 5 example).
#
# For citizen:
#   Non-citizens tend to be geographically clustered in the same PSUs
#   (urban areas, specific regions). High within-PSU homogeneity means a
#   high intra-cluster correlation (ICC), leading to HIGH DEFF. The
#   cluster design is particularly inefficient for variables that are
#   spatially concentrated.
#
# For env_steps:
#   Environmental attitudes are more evenly spread geographically.
#   Within-PSU homogeneity is lower, so ICC is lower, and DEFF should
#   be SMALLER than for citizenship. The design penalty is less severe
#   for attitudinal variables that do not cluster spatially.
#
# Neff (effective sample size = n / DEFF):
#   If DEFF = 2.0 for citizen with n = 2,204, then Neff = 1,102.
#   This means the complex sample gives the same precision as an SRS of
#   only ~1,102 people, despite having 2,204 respondents in the file.
#   The gap between n and Neff is the statistical "cost" of clustering.
#   A larger DEFF for citizen than for env_steps would show that
#   demographic variables bear a greater efficiency penalty from
#   cluster sampling than attitudinal variables do.
#
# ============================================================
# End of script
# ============================================================